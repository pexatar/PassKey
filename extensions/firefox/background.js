/**
 * PassKey Firefox Extension - Background Script (Event Page)
 *
 * Manages:
 * - Native Messaging connection to BrowserHost.exe via browser.runtime.connectNative
 * - ECDH session establishment for encrypted password retrieval
 * - Message routing between popup/content script and native host
 *
 * Firefox MV3 differences from Chrome:
 * - background.scripts array in manifest (not service_worker)
 * - Scripts loaded via manifest order, no importScripts() needed
 * - browser.* API (promise-based) — also supports chrome.* compat shim
 * - Event pages persist longer than Chrome service workers
 */
 ...
 *
 * Firefox difference: port.error.message is used instead of chrome.runtime.lastError
 * to retrieve the disconnect reason.
 *
 * @returns {runtime.Port|null} The active port, or null if the host is unavailable.
 */
function ensurePort() {
  if (port) return port;
  ...
  port.onDisconnect.addListener(() => {
    const error = port?.error?.message || 'native-disconnected';
    port = null;
    sessionId = null;
    sessionKey = null;
    keyPair = null;
...
// Firefox expects onMessage listeners to return a Promise (not use sendResponse callback).
browser.runtime.onMessage.addListener((msg, sender) => {
  // Firefox expects a Promise return (not sendResponse callback)
  return handleMessage(msg, sender)
    .catch(err => ({ success: false, error: err.message || 'internal-error' }));
});
 ...
 * @param {runtime.MessageSender} sender - Sender context (tab ID, etc.).
...
async function handleMessage(msg, sender) {
  switch (msg.type) {
    ...
    case 'detect-forms':
      return browser.tabs.sendMessage(msg.tabId, { type: 'detect-forms' });
...
async function updateBadgeForTab(tabId, url) {
  ...
  try {
    ...
    browser.action.setBadgeText({ text: count > 0 ? String(count) : '', tabId }).catch(() => {});
    if (count > 0) {
      browser.action.setBadgeBackgroundColor({ color: '#0078d4', tabId }).catch(() => {});
    }
  } catch {
    browser.action.setBadgeText({ text: '', tabId }).catch(() => {});
  }
