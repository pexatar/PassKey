...
// ─── SVG helper (CSP-safe: avoids .innerHTML) ────────────────────────────────

/**
 * Parses an SVG string and appends the resulting SVG element as the sole child
 * of the given element. Uses DOMParser instead of .innerHTML to comply with
 * the extension's Content Security Policy (script-src 'self').
 *
 * @param {Element} element - The container element to update.
...
function initStrings() {
  const t = window.t;
  // Set lang attribute dynamically so screen readers and CSS :lang() rules
  // reflect the actual locale (popup.html defaults to 'en' as static fallback).
...
async function init() {
  ...
  let statusResp;
  try {
    statusResp = await chrome.runtime.sendMessage({ type: 'get-status' });
  } catch {
    statusResp = null;
  }
 ...
 *
 * @param {chrome.tabs.Tab} tab - The active tab object.
 * @returns {Promise<void>}
 */
async function loadCredentialsAndShow(tab) {
  ...
  const [siteResp, allResp] = await Promise.all([
    isHttpUrl
      ? chrome.runtime.sendMessage({ type: 'get-credentials', url: tab.url }).catch(() => null)
      : Promise.resolve(null),
    chrome.runtime.sendMessage({ type: 'get-all-credentials' }).catch(() => null)
  ]);
  ...
  chrome.action.setBadgeText({ text: badgeCount > 0 ? String(badgeCount) : '' }).catch(() => {});
  if (badgeCount > 0) {
    chrome.action.setBadgeBackgroundColor({ color: '#0078d4' }).catch(() => {});
  }
...
/**
 * Requests the decrypted password from the background service worker (via ECDH),
...
async function onCopyPassword(cred, btn) {
  ...
  try {
    const resp = await chrome.runtime.sendMessage({ type: 'copy-credential', id: cred.id });
    if (resp.success) {
...
async function doUnlock() {
  ...
  let result;
  try {
    result = await chrome.runtime.sendMessage({ type: 'unlock-vault', masterPassword: pw });
  } catch {
    result = null;
  } finally {
...
btnOpenApp.addEventListener('click', async () => {
  await chrome.runtime.sendMessage({ type: 'show-window' }).catch(() => {});
});
