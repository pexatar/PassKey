 ...
 * service worker can correlate async responses in the pendingRequests Map.
 * The clientId is set to chrome.runtime.id when running inside the extension;
...
function buildRequest(action, payload = null) {
  return {
    ...
    clientId: typeof chrome !== 'undefined' && chrome.runtime ? chrome.runtime.id : 'unknown',
    payload: payload
  };
}
